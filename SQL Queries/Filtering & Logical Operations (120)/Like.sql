...
SELECT * FROM suppliers WHERE ContactName LIKE 'C%';
...
SELECT * FROM products WHERE QuantityPerUnit REGEXP '^[0-9]';
...
SELECT * FROM categories WHERE CategoryName LIKE '%ea%';

-- 12. Retrieve customers whose ContactName starts with 'M' and ends with 's' (ContactName LIKE 'M%s')
SELECT * FROM customers WHERE ContactName LIKE 'M%s'
